...
def fit_predict_model(city_data):
    ...
    gs = GridSearchCV(estimator=regressor, param_grid=parameters, scoring=scorer, cv=10)
    gs.fit(X,y)
    ...
    similar_house_index = np.argmin(similar_houses(x, X, reg.feature_importances_))
    similar_house_y = city_data.target[similar_house_index]
    np.set_printoptions(suppress=True)

    print "Most similar house: " + str(X[similar_house_index])
    print "Value of the most similar house: " + str(similar_house_y)
    print "Difference: " + str(similar_house_y - y)
